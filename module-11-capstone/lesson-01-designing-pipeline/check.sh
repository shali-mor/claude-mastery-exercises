...
echo "=== Lesson 01: Designing Pipeline checks ==="

for f in architecture.md cost-estimate.md; do
  F="${WORK}/${f}"
...
if [[ ! -f "$SJ" ]]; then bad "settings.json not found"; else
  ...
  grep -q "github\|mcpServers" "$SJ" && ok "GitHub MCP configured" || bad "Missing GitHub MCP config"
fi
