// ai.ts — Claude API client with prompt caching
import Anthropic from '@anthropic-ai/sdk';

const client = new Anthropic(); // reads ANTHROPIC_API_KEY from env

// Simulated large system prompt (in production this could be 10,000+ tokens of docs or tool defs)
const SYSTEM_PROMPT = `
...
export async function askWithCaching(userMessage: string): Promise<CachedResponse> {
  const response = await client.messages.create({
    model: 'claude-haiku-4-5-20251001', // using Haiku to keep exercise cost minimal
    max_tokens: 256,
    system: [
      {
        type: 'text',
        text: SYSTEM_PROMPT,
        // TODO: Add cache_control here to mark this block as cacheable.
        // Format: cache_control: { type: 'ephemeral' }
        // This tells Claude to cache this block on the first request and
        // read it from cache on subsequent requests at 10% of the normal cost.
